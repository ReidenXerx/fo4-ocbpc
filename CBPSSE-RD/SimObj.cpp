...
//	logger.error("bind\n");
    ...
    if (loadedData) {
        ...
        { std::vector<std::string> none; UpdateConfig(actor, none, config); }
        return  true;
    }
    return false;
}
...
void SimObj::UpdateStretch(Actor* actor) {
    ...
    if (smallest.empty() || !actor || !G::Root(actor) || !G::Root(actor))
        return;
    auto root = G::Root(actor);
    for (auto& t : things) {
        int g = (int)t.second.stretchGroup;
        if (g <= 0)
            continue;
        ...
        G::Local(child).pos = (m > 1e-4f && extra > 0.0f) ? across * (extra / m) : NiPoint3(0, 0, 0);
    }
}
